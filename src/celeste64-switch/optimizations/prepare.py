...
def optimize(out, port, game, foster, names):
    ...
    if 'srtmatrix' in names and 'affinemath' not in names:
        raise ValueError('srtmatrix requires affinemath')
    if 'posematrix' in names and 'affinemath' not in names:
        raise ValueError('posematrix requires affinemath')
    ...
    if 'scalarbindings' in names and 'matrixbindings' not in names:
        raise ValueError('scalarbindings requires matrixbindings')
    ...
    if 'textoutline' in names:
        from optimizations.textoutline import optimize_textoutline
        optimize_textoutline(out, port, override)
    ...
    if 'modelbits' in names:
        from optimizations.modelbits import optimize_modelbits
        optimize_modelbits(override)
    if 'proptransform' in names:
        from optimizations.proptransform import optimize_proptransform
        optimize_proptransform(override)
    ...
    if 'indexedcurves' in names:
        from optimizations.indexedcurves import optimize_indexedcurves
        optimize_indexedcurves(out, port, replace)
    ...
    if 'srtmatrix' in names:
        from optimizations.srtmatrix import optimize_srtmatrix
        optimize_srtmatrix(out, port, replace)
    if 'posematrix' in names:
        from optimizations.posematrix import optimize_posematrix
        optimize_posematrix(out, replace)
    ...
    if 'scalarbindings' in names:
        from optimizations.scalarbindings import optimize_scalarbindings
        optimize_scalarbindings(out, override, replace)
    ...
    if 'drawableframe' in names:
        from optimizations.drawableframe import optimize_drawableframe
        optimize_drawableframe(out, port, override)
    if 'nativecull' in names:
        if 'frustum' not in names:
