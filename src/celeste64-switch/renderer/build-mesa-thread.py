...
devkit = Path(os.environ['DEVKITPRO'])
...
identity = {'inputs': inputs, 'scripts': fingerprints, 'large_uploads': large, 'compiler': subprocess.check_output([str(devkit / 'devkitA64/bin/aarch64-none-elf-gcc'), '--version'], text=True), 'libnx_sha256': hashlib.sha256((devkit / 'libnx/lib/libnx.a').read_bytes()).hexdigest()}
...
if not (build / 'build.ninja').exists():
    subprocess.run([str(devkit / 'meson-cross.sh'), 'switch', str(out / 'cross-thread.ini'), str(build), str(source), '-Db_ndebug=true'], check=True)
